...
condorDir='/uscms/home/jmanagan/nobackup/YR_Delphes/Delphes342pre15_logs/' # Change username, helps to match log directory to the ROOT file directory, adding "_logs" (for compatibility with error checker)
...
fileList = [  # CHOOSE SAMPLES, you MUST have listed the file names with listFiles.py
    #'ZJetsToNuNu_HT-400To600_14TeV-madgraph.txt',
    #'ZZTo4L_14TeV_powheg_pythia8.txt',
    ]
...
for sample in fileList:
    ...
    for ifile, file in enumerate(rootfiles):
        ...
        print relPath+'_'+str(tempcount),'=',n_jobs,'jobs'
...
universe = vanilla
Executable = %(RUNDIR)s/GENtoDelphesDESY.sh
Should_Transfer_Files = YES
WhenToTransferOutput = ON_EXIT
Output = %(FILEOUT)s.out
Error = %(FILEOUT)s.err
Log = %(FILEOUT)s.log
Requirements = (TotalCpus == 8)
Notification = Never
